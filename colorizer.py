...
def make_block_3conv(layers, in_channels, out_channels, kernel_size=3, dilation=1, stride=1, padding=1):
    ...
    layers.append(nn.ReLU(inplace=True))
    layers.append(nn.Conv2(in_channels, out_channels, kernel_size, dilation, stride, padding))
    layers.append(nn.ReLU(inplace=True))
    layers.append(nn.BatchNorm2d(out_channels))
...
class Colorizer(nn.Module):
    def __init__(self):
        ...
        self.layers.append(nn.ConvTranspose2d(512, 216, 4, 2, 1))
        self.layers.append(nn.ReLU(True))
        make_block_2conv(self.layers, 256, 256)
